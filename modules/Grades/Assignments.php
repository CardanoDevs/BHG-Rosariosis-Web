...
if ( ! empty( $_POST['tables'] ) )
{
	...
	foreach ( (array) $_REQUEST['tables'] as $id => $columns )
	{
		...
		if ( ! $error && $go )
		{
			...
			if ( isset( $_FILES['assignment_file'] ) && count($_FILES['assignment_file']['name'])>0 )
			{
				
				//DBQuery( "DELETE FROM GRADEBOOK_ASSIGNMENT_FILES 
				//			WHERE ASSIGNMENT_ID=$id" );
							
							
				for($idx=0; $idx<count($_FILES['assignment_file']['name']); $idx++){
					
					$file_id = DBSeqNextID( 'GRADEBOOK_ASSIGNMENT_FILES_SEQ' );
					...
					if ( $file )
					{
						DBQuery( "INSERT INTO GRADEBOOK_ASSIGNMENT_FILES (ASSIGNMENT_FILE_ID,ASSIGNMENT_ID,FILE)
							VALUES(".$file_id.",$id,'$file');" );
					}
...
if ( ! $_REQUEST['modfunc'] )
{
	...
	echo '<script>
		$(document).ready(function(){
			$(\'#'.GetInputID('tables[' . $_REQUEST['assignment_id'] . '][SUBMISSION]').'\').click(function(){
			    console.log($(\'span:contains("Due")\'));
				if($(this).prop("checked") == true){
					//console.log($("#monthSelect2").prop("required"));
					$("#monthSelect2").prop("required", true);
					$("#daySelect2").prop("required", true);
					$("#yearSelect2").prop("required", true);
					$(\'span:contains("Due")\').prop("class", "legend-red");
				}
				else if($(this).prop("checked") == false){
					//console.log($("#monthSelect2").prop("required"));
					$("#monthSelect2").prop("required", false);
					$("#daySelect2").prop("required", false);
					$("#yearSelect2").prop("required", false);
					$(\'span:contains("Due")\').prop("class", "");
				}
			});
	...
	if ( ! empty( $_REQUEST['assignment_id'] ) )
	{
		...
		if($_REQUEST['assignment_id']!='new'){
			...
			if ( ! empty( $assn_RET ) )
			{
				...
				foreach ( (array) $assn_RET as $key => $value )
				{
					...
					$file_download = GetAssignmentFileLink( $value['FILE'] );
					$header.= ( $file_download ? $file_download . '<br />' : '' );
				}
		...
		$header.='<div class="multi-field-wrapper">
		...
		$header .= '<td>' . DateInput(
			$new && Preferences( 'DEFAULT_ASSIGNED', 'Gradebook' ) == 'Y' ?
				DBDate() :
				( empty( $RET['ASSIGNED_DATE'] ) ? date('Y-m-d') : $RET['ASSIGNED_DATE'] ),
		...
		$header .= '<td>' . CheckboxInput(
			...
			'',
			$_REQUEST['assignment_id'] == 'new', 'Yes', 'No', false
		) . '</td>';
