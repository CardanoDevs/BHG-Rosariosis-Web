	...
	if ( empty( $_REQUEST['id'] ) )
	{
		...
		if ( ! empty( $courses_RET ) )
		{
			...
			foreach ( (array) $courses_RET as $course_period_id => $course )
			{
				...
				$sql = "SELECT s.STUDENT_ID,gt.ASSIGNMENT_TYPE_ID,sum(CASE WHEN gg.POINTS ='-1' THEN '0' WHEN gg.POINTS='0' THEN '0'  ELSE gg.POINTS END) AS PARTIAL_POINTS,sum(CASE WHEN gg.POINTS ='-1' THEN '0' WHEN gg.POINTS IS NULL THEN '0' ELSE ga.POINTS END) AS PARTIAL_TOTAL,gt.FINAL_GRADE_PERCENT,sum(" . db_case( array( 'gg.POINTS', "''", "1", "0" ) ) . ") AS UNGRADED
				FROM STUDENTS s
				...
				foreach ( (array) $assignments_RET as $assignment )
				{
					...
					$LO_ret[] = array(
						...
						'PERCENT' => ( $assignment['POINTS_POSSIBLE'] == '0' ?
							...
							( $assignment['POINTS'] == '-1' ?
								'*' :
								number_format( 100 * $assignment['POINTS'] / $assignment['POINTS_POSSIBLE'] ) . '%' ) ),
