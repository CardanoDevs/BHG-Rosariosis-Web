...
if ( ! function_exists( 'ReportCardsIncludeForm' ) )
{
    ...
    function ReportCardsIncludeForm( $include_on_title = 'Include on Report Card', $mailing_labels = true )
    {
        ...
        $teacherss = DBGet( "SELECT first_name,last_name,middle_name FROM staff
        ...
        $return .= '<tr class="st"><td><label><input type="checkbox" name="elements[cumgpa]" value="Y" /> ' .
            _( 'CUM GPA' ) . '</label></td> ';
        $return .= '<td><label><input type="checkbox" name="elements[termpercent]" value="Y" /> ' .
            _( 'Term Percent' ) . '</label></td></tr>';
        $return .= '<tr class="st"><td><label><input type="checkbox" name="elements[sidegpa]" value="Y" /> ' .
            _( 'Side GPA' ) . '</label></td>';
        $return .= '<td><label><input type="checkbox" name="elements[termgpa]" value="Y" /> ' .
            _( 'Term GPA' ) . '</label></td></tr>';
        $return .= '<tr class="st"><td><label><input type="checkbox" name="elements[exam]" value="Y" /> ' .
            _( 'Exam' ) . '</label></td>';
        $return .= '<td><label><input type="checkbox" name="elements[signature]" value="Y" /> ' .
            _( 'Signature' ) . '</label></td></tr>';
        $return .= '<tr class="st"><td><label><input type="checkbox" name="elements[progress]" value="Y" /> ' .
            _( 'Progress' ) . '</label></td>';
        $return .= '<td><label><input type="checkbox" name="elements[classr]" value="Y" /> ' .
            _( 'Class Rank' ) . '</label></td></tr>';
        $return .= '<tr class="st"><td><label> ' .
            _( 'HR Teachers' ) . ':</label> <br><select multiple name="elements[hrteacher][]" id="hrteacher">';


        foreach ( (array) $teacherss as $prof)
        {
            $return .= '<option value="' . $prof['FIRST_NAME'] . ' '.$prof['MIDDLE_NAME'].' '.$prof['LAST_NAME'].'">' . $prof['FIRST_NAME'] . ' '.$prof['MIDDLE_NAME'].' '.$prof['LAST_NAME'].' </option>';
        }
...
font-size:15px !important;
    ...
    .headerb{display:none !important;}
    ...
    border: 1px solid black;
        ...
        if(isset($_REQUEST['elements']['exam'])) {
            $LO_columns['EXAM'] = 'Exam';
        }
        if ( $_REQUEST['elements']['comments'] === 'Y' )
        {
        ...
        if($_REQUEST['elements']['sidegpa'] === 'Y') {
            $LO_columns['GPA'] = 'GPA';
        }
        // Report Cards array.
        $report_cards = array();
        ...
        foreach($student_RET as $student_id => $course_periods) {
            foreach ((array)$course_periods as $course_period_id => $mps) {



                foreach ($mp_array as $mpx) {

                    $total_grade_point = 0;
                    $Total_Credit_Hr_Attempted = 0;
                    $chek = DBGet(DBQuery('SELECT marking_period_id, MP from school_marking_periods WHERE marking_period_id = ' . $mpx));

                    if ($chek[1]['MP'] == 'QTR') {
                        $qtre = 1;
                        $qtrx = $chek[1]['MARKING_PERIOD_ID'];

                    }
                    if ($chek[1]['MP'] == 'SEM') {
                        $seme = 1;
                        $semx = $chek[1]['MARKING_PERIOD_ID'];
                    }

                    if ($seme == 1 and $qtre == 1) {
                        //echo '1';
                        //echo'<br>'.$semx.'<br>';
                        $fukte = DBGet(DBQuery('SELECT UNWEIGHTED_GP, CREDIT_ATTEMPTED FROM student_report_card_grades WHERE student_id=' . $student_id . ' AND marking_period_id=\'' . $semx . '\' AND course_title=\'' . $grades_RET[$i]['COURSE_TITLE'] . '\''));
                        $kil = $mpx;
                    } elseif ($qtre == 1 and $seme != 1) {
                        //echo '2';
                        $fukte = DBGet(DBQuery('SELECT UNWEIGHTED_GP, CREDIT_ATTEMPTED FROM student_report_card_grades WHERE student_id=' . $student_id . ' AND marking_period_id =\'' . $qtrx . '\' AND course_title=\'' . $grades_RET[$i]['COURSE_TITLE'] . '\''));
                        $kil = $mpx;
                    } elseif ($seme == 1 and $qtre != 1) {
                        // echo 3;
                        $fukte = DBGet(DBQuery('SELECT UNWEIGHTED_GP, CREDIT_ATTEMPTED FROM student_report_card_grades WHERE student_id=' . $student_id . ' AND marking_period_id =\'' . $semx . '\' AND course_title=\'' . $grades_RET[$i]['COURSE_TITLE'] . '\''));
                        $kil = $mpx;
                    }
                ...
                if ($seme == 1 and $qtre == 1) {
                    ...
                    if (count($mps[$kil][1]['WEIGHTED_GP']) != 0) {

                        $total_grade_point += ($mps[$kil][1]['WEIGHTED_GP'] * $mps[$kil][1]['CREDIT_ATTEMPTED']);
                        $Total_Credit_Hr_Attempted += $mps[$kil][1]['CREDIT_ATTEMPTED'];
                    } elseif (count($mps[$kil][1]['UNWEIGHTED_GP']) != 0) {
                        $total_grade_point += ($mps[$kil][1]['UNWEIGHTED_GP'] * $mps[$kil][1]['CREDIT_ATTEMPTED']);
                        $Total_Credit_Hr_Attempted += $mps[$kil][1]['CREDIT_ATTEMPTED'];
                    }
                } else {
                ...
                if ($Total_Credit_Hr_Attempted != 0) {
                    $termgpax[$student_id][] = sprintf("%01.3f", ($total_grade_point / $Total_Credit_Hr_Attempted));
                } else {
                    $termgpax[$student_id][] = 0;
                }
                $total_grade_point = 0;
                $Total_Credit_Hr_Attempted = 0;
        ...
        foreach($termgpax as $keie => $student){
            $cevaxx=0;
            $countxxa=0;

            foreach($student as $suma){
                $cevaxx+=$suma;
                $countxxa++;
            }
            $termgpxx[$keie]=$cevaxx/$countxxa;
            $countxxa=0;
        }
        ...
        */
        ...
        foreach ( (array) $student_RET as $student_id => $course_periods )
        {
            ...
            foreach ( (array) $course_periods as $course_period_id => $mps )
            {
                ...
                if(!empty( $mps[key( $mps )][1]['COURSE_TITLE'])) {
                ...
                foreach($mps as $idx){
                    if(!empty($idx[1]['EXAM'])) {
                        if(isset($_REQUEST['elements']['exam'])) {
                            $grades_RET[$i]['EXAM'] = $idx[1]['EXAM'] . '%';
                        }
                    }}



                //   $grades_RET[$i]['EXAM'] =GetMP( $mps, 'SHORT_NAME' );
                ...
                if($seme == 1 and $qtre==1) {
                    ...
                    if (count($mps[$kil][1]['WEIGHTED_GP'])!=0) {
                        ...
                        $total_grade_point += ($mps[$kil][1]['WEIGHTED_GP'] * $mps[$kil][1]['CREDIT_ATTEMPTED']);
                        $Total_Credit_Hr_Attempted += $mps[$kil][1]['CREDIT_ATTEMPTED'];
                    } elseif (count($fukte[1]['UNWEIGHTED_GP'])!=0) {
                        $total_grade_point += ($fukte[1]['UNWEIGHTED_GP'] * $fukte[1]['CREDIT_ATTEMPTED']);
                        $Total_Credit_Hr_Attempted += $fukte[1]['CREDIT_ATTEMPTED'];
                    }
                }else{
                    ...
                    if (count($mps[$kil][1]['WEIGHTED_GP'])!=0) {
                        $total_grade_point += ($mps[$kil][1]['WEIGHTED_GP'] * $mps[$kil][1]['CREDIT_ATTEMPTED']);
                        $Total_Credit_Hr_Attempted += $mps[$kil][1]['CREDIT_ATTEMPTED'];
                    } elseif (count($fukte[$kil][1]['UNWEIGHTED_GP'])!=0) {
                        $total_grade_point += ($fukte[1]['UNWEIGHTED_GP'] * $fukte[1]['CREDIT_ATTEMPTED']);
                        $Total_Credit_Hr_Attempted += $fukte[1]['CREDIT_ATTEMPTED'];
                    }
                }
                ...
                if($_REQUEST['elements']['sidegpa'] === 'Y') {
                    if($Total_Credit_Hr_Attempted!=0) {
                        $grades_RET[$i]['GPA'] = number_format($total_grade_point / $Total_Credit_Hr_Attempted, 2);
                    }else{
                        $grades_RET[$i]['GPA'] = 0;
                ...
                if($Total_Credit_Hr_Attempted!=0) {
                    $termgpa[] = sprintf("%01.3f", ($total_grade_point / $Total_Credit_Hr_Attempted));
                }else{
                    $termgpa[]=0;
                }
                $total_grade_point=0;
                ...
                foreach ( (array) $mp_array as $mp )
                {
                    ...
                    if(GetMP($mp,'MP')=='SEM') {
                        $percentsx[] = $mps[$mp][1]['GRADE_PERCENT'];
                    }
                    // Comments.
            ...
            foreach ( (array) $addresses as $address )
            {
                ...
                // Headers.
                // FJ add school logo.
                if(isset($_REQUEST['elements']['progress'])){
                    $logo_pic = 'assets/progress_'.UserSchool().'.jpg';
                }else {
                ...
                $megpa=0;
                $xx=0;
                foreach($termgpa as $gpx){
                    $megpa+=$gpx;
                    $xx++;
                }
                $megpa=$megpa/$xx;
                DBQuery( "UPDATE STUDENT_MP_STATS SET cum_weighted_factor=".($megpa/SchoolInfo( 'REPORTING_GP_SCALE' ))."
			WHERE student_id='" . $student_id . "'
			AND marking_period_id='" . $kil . "'" );


                //DrawHeader( $student_grade_level, SchoolInfo( 'TITLE' ) );

                $sem  = DBGet( "SELECT marking_period_id FROM school_marking_periods WHERE school_id=".UserSchool()." AND mp='SEM'");

                $plax=array();


                foreach($sem as $semestru){
                    $total_grade_point=0;
                    $Total_Credit_Hr_Attempted=0;
                    $doex = DBGet( "SELECT WEIGHTED_GP,UNWEIGHTED_GP, CREDIT_ATTEMPTED FROM student_report_card_grades WHERE school_id=".UserSchool()." AND student_id=".$student_id." AND marking_period_id='".$semestru['MARKING_PERIOD_ID']."'");

                    foreach($doex as $curs) {
                        if(!empty($curs['WEIGHTED_GP'])){  // this might error(give wrong numbers)- an4rei TODO
                            $total_grade_point += ($curs['WEIGHTED_GP'] * $curs['CREDIT_ATTEMPTED']);
                            $Total_Credit_Hr_Attempted += $curs['CREDIT_ATTEMPTED'];
                        }else {
                            $total_grade_point += ($curs['UNWEIGHTED_GP'] * $curs['CREDIT_ATTEMPTED']);
                            $Total_Credit_Hr_Attempted += $curs['CREDIT_ATTEMPTED'];
                        }
                    }
                    if($Total_Credit_Hr_Attempted!=0) {
                        if(($total_grade_point / $Total_Credit_Hr_Attempted)!=0) {
                            $plax[$semestru['MARKING_PERIOD_ID']][] = round($total_grade_point / $Total_Credit_Hr_Attempted, 2);
                        }
                    }
                }
                ...
                $obiecte=[];
                foreach($_REQUEST['elements'] as $key => $cur){
                    if($key=='cumgpa'){
                        $obiecte[]=$key;
                    }elseif($key =='termgpa'){
                        $obiecte[]=$key;
                    }elseif($key == 'termpercent'){
                        $obiecte[] = $key;
                    }elseif($key == 'hrteacher'){
                        $obiecte[] = $key;
                    }elseif($key == 'classr'){
                        $obiecte[] = $key;
                    }
                ...
                $deselet=array();
                if (in_array( 'termgpa',$obiecte) and !in_array('termgpa',$deselet)) {
                    $field = 'Term GPA: ' . number_format(round($megpa, 2), 2);
                    $deselet[]='termgpa';
                }elseif (in_array( 'termpercent',$obiecte) and !in_array('termpercent',$deselet)) {
                    $field = 'Term Percent: ' . number_format(array_sum($percentsx)/count($percentsx), 0) . '%';
                    $deselet[]='termpercent';
                }elseif (in_array( 'cumgpa',$obiecte) and !in_array('cumgpa',$deselet)) {
                    $field = 'CUM GPA: ' . number_format(round($cumgp, 2), 2);
                    $deselet[]='cumgpa';
                }elseif (in_array( 'classr',$obiecte) and !in_array('classr',$deselet)) {
                    $field = 'Class Rank: ' . $classrank[$student_id];
                ...
                DrawHeader( 'Student Name: '. $student_full_name , $field);



                if(isset($_REQUEST['elements']['hrteacher'])){
                    if (in_array( 'termgpa',$obiecte) and !in_array('termgpa',$deselet)) {
                        $field = 'Term GPA: ' . number_format(round($megpa, 2), 2);
                        $deselet[]='termgpa';
                    }elseif (in_array( 'termpercent',$obiecte) and !in_array('termpercent',$deselet)) {
                    ...
                    $profi= rtrim(implode(', ', $_REQUEST['elements']['hrteacher']), ',');

                    DrawHeader( 'HR Teachers: '. $profi,$field );
                ...
                DrawHeader('Student ID: ' . $student_id,$field);
                ...
                DrawHeader("Student Grade: " . $student_grade_level,$field );
            ...
            {
               echo '<style>.padd{padding:15px;}</style>';
                echo '<br/>';
                echo '<span style="font-size:10px; font-weight:bold;"></span>';
                echo '<br/>';
                echo '<table width="100%" border=0>';
                echo '<tr></tr><tr><td align=left><span style="font-size:12px; font-weight:bold; height:25px;"><br>Principal&rsquo;s Signature</span><br/><br/></td><td>:</td><td class="padd"  ><img style="max-width:11%" src="assets/principal_'.UserSchool().'.jpg"></td>';
				echo '<tr></tr><tr><td align="left" width=190><span style="font-size:12px; font-weight:bold; height:20px;"></span><br/><br/><br/></td><td  width="5" ></td><td align=left class="padd"></td></tr>';
                echo '<tr></tr><tr><td align="left" width=190><span style="font-size:12px; font-weight:bold; height:20px;"></span><br/><br/></td><td width="5" ></td><td align=left  class="padd"  ></td><td valign=top align=left><span style="font-size:12px; font-weight:bold; height:20px;"></span></td></tr>';
                echo '<td valign=top align=left><span style="font-size:12px; font-weight:bold; height:20px;"></span></td></tr>';
                echo '<tr></tr><tr><td align="left"><span style="font-size:12px; font-weight:bold; height:20px;"></span></td><td></td><td class="padd" ></td><td align=left><span style="font-size:12px; font-weight:bold; height:20px;"></span></td></tr>';
                echo '</table>';
...
function GetReportCardsExtra( $mp_list, $st_list )
{
    ...
    $extra['SELECT_ONLY'] .= ",sg1.GRADE_LETTER as GRADE_TITLE,sg1.GRADE_PERCENT, sg1.CREDIT_ATTEMPTED AS CREDIT_ATTEMPTED, sg1.EXAM as EXAM, sg1.WEIGHTED_GP as WEIGHTED_GP,
		sg1.COMMENT as COMMENT_TITLE,sg1.STUDENT_ID,sg1.COURSE_PERIOD_ID,sg1.MARKING_PERIOD_ID,sg1.UNWEIGHTED_GP as UNWEIGHTED_GP,
