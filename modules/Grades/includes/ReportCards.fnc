    ...
    border: 1px solid black;
        ...
        foreach ( (array) $student_RET as $student_id => $course_periods )
        {
            ...
            foreach ( (array) $addresses as $address )
            {
                ...
                DrawHeader( SchoolInfo( 'TITLE' ) . '('.FormatSyear( UserSyear(), Config( 'SCHOOL_SYEAR_OVER_2_YEARS' ) ).')' );
			...
			AND marking_period_id='" . $kil . "'" );
                ...
                DrawHeader( 'Student Name: '. $student_full_name , 'Student Grade: ' . $student_grade_level  );
                if(isset($_REQUEST['elements']['termgpa'] )) {
                    DrawHeader('Student ID: ' . $student_id, 'Term GPA: ' . number_format(round($megpa, 2), 2));
                }else{
                    DrawHeader('Student ID: ' . $student_id);
                ...
                if($_REQUEST['elements']['termpercent'] === 'Y') {
                    if(count($percentsx)>0) {
                        DrawHeader('', 'Term Percent: ' . number_format(array_sum($percentsx) / count($percentsx), 0) . '%');
                    }
                }
                if($_REQUEST['elements']['cumgpa'] === 'Y') {
                    DrawHeader(_('School Year') . ': ' . FormatSyear(UserSyear(), Config('SCHOOL_SYEAR_OVER_2_YEARS')), 'CUM GPA: ' . number_format(round($cumgp, 2), 2));
                }else{
            ...
            if($_REQUEST['elements']['signature']=='Y' )
            {
                ...
                echo '<tr><td align="left" width=190><span style="font-size:12px; font-weight:bold; height:20px;">Teacher&rsquo;s Comment</span><br/><br/><br/></td><td width="5" valign=top>:</td><td align=left  valign=top>_________________________________________________________________________________________</td></tr>';
                echo '<tr><td align="left" width=190><span style="font-size:12px; font-weight:bold; height:20px;">Teacher&rsquo;s Signature</span><br/><br/></td><td width="5" valign=top>:</td><td align=left  valign=top>______________________________________</td><td valign=top align=left><span style="font-size:12px; font-weight:bold; height:20px;"></span></td></tr>';
                echo '<tr><td align=left><span style="font-size:12px; font-weight:bold; height:20px;">Principal&rsquo;s Signature</span><br/><br/></td><td  valign=top>:</td><td  valign=top>______________________________________</td>';
                echo '<td valign=top align=left><span style="font-size:12px; font-weight:bold; height:20px;"></span></td></tr>';
                echo '<tr><td align="left"><span style="font-size:12px; font-weight:bold; height:20px;">Parent/Guardian&rsquo;s Signature</span></td><td  valign=top>:</td><td valign=top>______________________________________</td><td valign=top align=left><span style="font-size:12px; font-weight:bold; height:20px;"></span></td></tr>';
                echo '</table>';
